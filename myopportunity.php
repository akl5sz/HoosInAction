...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['delbtn'])) {
        deleteSignUp($_POST['memberID'], $_POST['organizationID'], $_POST['Date'], $_POST['Time']);
    }
  }
if($_SESSION['memberID']){
    $id = $_SESSION['memberID'];
    $list_of_opportunities = getUserSignUp($id);
    ...
    <div class="album py-5 bg-body-tertiary">
            ...
            <table class="table">
                ...
                <?php foreach ($list_of_opportunities as $opportunity) : ?>
                    <tr>
                        <!-- `studentID`, `organizationID`, `Date`, `Time`, `Location` -->
                        <td><?php echo $opportunity['studentID']; ?></td>
                        <td><?php echo $opportunity['organizationID']; ?></td>
                        <td><?php echo $opportunity['Date']; ?></td>
                        <td><?php echo $opportunity['Time']; ?></td>
                        <td><?php echo $opportunity['Location']; ?></td>
                        <td><?php echo '
                        <form name="deleteform" action="myopportunity.php" method="post">
                        <input type="text" name="memberID" value="' . $opportunity['studentID'].'" hidden>
                        ...
                        <input type="time" name="Time" value="' . $opportunity['Time'].'" hidden>
                        <input type="submit" value="Delete" name="delbtn" class="btn btn-primary"/>
                        </form>';
                        ?>
                        </td>
                    </tr>
